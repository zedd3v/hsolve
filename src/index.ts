...
const hsl = async (req: string) => {
    try {
        ...
        return new Promise((resolve, reject) => {
            VM.runInNewContext(`var self={};function atob(a){return Buffer.from(a,'base64').toString('binary')} ${body} hsl('${req}').then(resolve).catch(reject)`, {
                Buffer,
                resolve,
                reject,
            });
...
const hsolve = async (url: string, options: { timeout?: number; solveService?: SolveService; proxy?: string; } = {}): Promise<string> => {
    try {
        ...
        const timeoutCheck = timeout !== null ? setInterval(() => {
        ...
        const solution = await submitCaptcha(hostname, siteKey, userAgent, (Date.now() + _.random(30, 120)), captchaTask.key, captchaTask.request_type, answers, agents);
